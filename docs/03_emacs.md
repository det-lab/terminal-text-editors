...
On macOS and Linux, `Emacs` is typically installed by default. For users running WSL, you can install it from the terminal by running:
```bash
...
## Basic Commands:
`Emacs` commands will generally involve the CONTROL (CTRL or CTL) key and the META key (sometimes EDIT or ALT).
### Open a File
...
* Press `CTRL+S` or `CTRL+R` again to continue searching in the same direction.
...
* To use **regex** instead, type `META+CTRL+S` (`ALT+CTRL+S`). If you're unfamiliar with using regex, [here is a lesson on using regex](https://carpentries-incubator.github.io/regex-novice-biology/), and [here is a website for testing your expressions](https://regex101.com/).
